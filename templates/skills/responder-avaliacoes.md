...
o cliente recebe avaliação de cliente com regularidade (Google Meu Negócio e
similares) e hoje responde na mão, sem padrão. Adaptar ao caso descrito antes
de salvar em .claude/skills/responder-avaliacoes/SKILL.md — nunca sem
...
description: >
  Escreve respostas curtas e humanas pras avaliações do Google Meu Negócio (ou
  plataforma similar). Mantém o padrão: nome do cliente, agradecimento
  variado, frase concreta sobre o produto/serviço da empresa, emoji opcional.
  Nada de resposta automática de empresa grande. Use quando o usuário disser
  "responder avaliação", "resposta pro Google", "tem uma avaliação nova", "me
  ajuda a responder essas reviews", "avaliação do GMB", ou colar texto/print
  de uma review.
---

# /responder-avaliacoes — Respostas pras avaliações
...
Listar cada resposta abaixo da review, pronta pra colar na plataforma. Formato:
...
Se o usuário pedir pra guardar, salvar em `avaliacoes-google/YYYY-MM-DD-respostas.md` com review + resposta publicada. Só criar a pasta se o usuário quiser registro. Não fazer por padrão.
