...
# /atualizar — Manutenção de Memória

## O que fazer

Fazer uma varredura comparando o **estado real do workspace** com o que está **documentado nos arquivos de memória**. Identificar diferenças e propor atualizações pro usuário aprovar.
...
1. `CLAUDE.md` — foco em: estrutura de pastas, sobre a empresa, ferramentas conectadas (seções de topo — não as regras fixas da constituição)
2. `_memoria/empresa.md` — foco em: equipe, ferramentas, entregas, departamentos
3. `_memoria/estrategia.md` — foco em: prioridade principal, fase do rollout, o que pode esperar
4. `_memoria/preferencias.md` — foco em: tom de voz, o que evitar
5. `_memoria/agora.md` — contexto vivo: onde paramos, decisões recentes, pendências, o que está quente
6. `marca/design-guide.md` — foco em: cores, fontes, estilo

## Passo 2.5: Atualizar o contexto vivo (`agora.md`)
...
Olhando o que rolou nesta sessão, atualizar `_memoria/agora.md`:
...
- **Higiene:** o que passou de ~30 dias em "Decisões recentes" e "Quente agora" sai daqui (some, ou vai pra um `_memoria/historico.md` se o usuário quiser guardar). Manter o arquivo curto.
...
Para cada arquivo de memória, verificar:

### CLAUDE.md (seções de topo, não as regras fixas)
...
- Departamentos/áreas ou clientes mencionados ainda são atuais?
- Equipe mudou?

### _memoria/estrategia.md
- A fase do rollout (piloto/departamental/empresa toda) ainda bate com a realidade, ou já avançou?
- A prioridade principal ainda parece ser o foco (baseado nos arquivos recentes)?
- Tem prazos vencidos ou contextos datados?

### _memoria/preferencias.md
- (Raramente muda, mas verificar se tem algo contraditório com o uso recente)
...
> "Tudo atualizado. Os arquivos de memória refletem o estado atual do workspace."

## Passo 5: Aplicar as correções (com aprovação)

Para cada item desatualizado, mostrar a mudança proposta:

> **`_memoria/empresa.md`** — adicionar "Notion" na lista de ferramentas
>
...
- Se a instalação acabou de ser configurada (instalar recente, poucos commits), dizer que está tudo certo e não forçar atualizações desnecessárias
