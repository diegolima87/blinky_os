...
name: syncar
description: >
  Salva o estado atual do workspace no GitHub do cliente (commit + push).
...
Se `git remote get-url origin` não retornar nada, esse repositório ainda não está conectado ao GitHub.

Diga:

> "Esse workspace ainda não está conectado a um repositório no GitHub.
>
> Pra conectar, você (ou o cliente) precisa de um repositório dedicado — cada empresa cliente tem o seu, separado dos demais. Se ainda não criou:
> 1. Acesse github.com/new
> 2. Crie um repositório privado (nome sugerido: nome da empresa cliente)
> 3. Não inicialize com README — deixa vazio
> 4. Me passa o link do repositório criado (ex: https://github.com/organizacao/nome-do-cliente)"
...
> "Conectado. O workspace dessa empresa está agora em [link].
> Sempre que quiser salvar, é só pedir 'synca' (ou rodar /syncar) que eu faço o commit e o push."
...
Para a descrição do commit, use o que foi feito na sessão (ex: "sync: nova proposta cliente X", "sync: skill de relatório de status", "sync: atualização de memória"). Se não souber o que colocar, use `sync: atualizações do dia`.

Após o push, confirme:

> "Salvo. O trabalho está seguro em [url do remote]."
...
>
> Se quiser resolver agora, me diz e eu ajudo passo a passo."
...
- Antes do primeiro `/syncar`, checar `git status` pra garantir que nenhum arquivo sensível de dado real de cliente está prestes a subir sem intenção (ver seção Confidencialidade do `CLAUDE.md`)
